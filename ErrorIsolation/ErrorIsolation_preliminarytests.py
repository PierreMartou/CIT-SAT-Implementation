...
def debugging_z3solver():
    models = "../data/RIS-FOP/"
    feature_model_path = models + 'features.txt'
...
def find_suspects_SPLOT(original_suite, alternatives, step):
    ...
    if step == 0:
        ...
        return all_combinations(activations, deactivations)
    else:
        prev = original_suite.getUnorderedTestSuite()[step-1]

        activations = [f for f in prev if prev[f] < 0 and original_next[f] > 0]
        deactivations = [f for f in prev if prev[f] > 0 and original_next[f] < 0]
        suspects = suspects + all_combinations(activations, deactivations)

        alt_paths = alternatives[step-1]

        if len(alt_paths) == 0:
            return suspects

        #biggest_alt_path = max(alt_paths, key=lambda ts: len(ts.getUnorderedTestSuite()))
...
def group_initialisation(suspects, system_data):
    groups = []
    while suspects:
        t = BuildingCTT(system_data, verbose=False, limit=0, numCandidates=3, weight_lookahead=1, specificTransitionCoverage=suspects)
