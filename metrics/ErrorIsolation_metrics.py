import math
import os
import ErrorIsolation

from ErrorIsolation.ErrorIsolation import ErrorIsolationStatistics, getErrorIsolation
...
def average_de_activations(max_iterations=5, recompute=False):
    ...
    for filename in os.listdir(modelFiles):
        ...
        error_isolation = getErrorIsolation(tempStorageErrorIsolation, s, tempStorageCTT, tempstorageAlts, verbose=verbose, recompute=recompute)
    ...
    average_suspects = aggregate_data(average_cost, step=10)
    r2_score(average_suspects)
    r2_score(average_suspects, degree=2)
    plot_dict_with_regression(average_suspects, "Average number of (de)activations)", degree=2)
...
def overall_isolation_metrics(max_iterations = 10, group_mode = None, recompute=False):
    modelFiles = "../data/SPLOT/SPLOT-NEW/SPLOT-txt/"
    constraintsFiles = "../data/SPLOT/SPLOT-NEW/SPLOT-txtconstraints/"
    storageCTT = "../data/SPLOT/SPLOT-NEW/SPLOT-TestSuitesCTT/"
    storageAlts = "../data/SPLOT/SPLOT-NEW/SPLOT-Alts/"
    ...
    for filename in os.listdir(modelFiles):
        ...
        error_isolation = getErrorIsolation(tempStorageErrorIsolation, s, tempStorageCTT, tempstorageAlts,
                                            verbose=verbose, group_mode=group_mode, recompute=recompute)
        ...
        for iteration in range(max_iterations):
            ...
            new_statistics = error_isolation.get_overall_statistics(nb_errors=1, iteration=iteration, states=10, recompute=recompute, only_if_exists=only_if_exists)
...
def table_all_stats_SPLOT(max_iterations=10, recompute=False):
    all_statistics = overall_isolation_metrics(max_iterations, None, recompute)
...
def plotgroupSPLOT(metric = "steps", max_iterations=5, recompute=False):
    all_statistics1group = overall_isolation_metrics(max_iterations, 1, recompute=recompute)

    all_statisticsnormal = overall_isolation_metrics(max_iterations, None, recompute=recompute)
    ...
    if metric == "Size":
        print("\nWith group initialisation: ", degreenormal, " degree, R2 : ", r2certainty_normal)
        print("Without (1 group): ", degree1group, " degree, R2 : ", r2certainty_1group)
